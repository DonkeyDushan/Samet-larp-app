...
2. **Schéma databáze** v Drizzle, pokrývající:
   ...
   - `questions`, `answer_options` — otázky jsou **vlastní pro každou postavu**, žádná sdílená sada. Odpověď nese dopady na škály, **seznam bloků, které zapíná**, a **příznaky, které nastavuje** (§4.5, vrstva 1 a 2). Otázka má navíc pole **`zdroj`: `hráč` nebo `org`** — organizátorské otázky se netisknou do dotazníku a vyplňuje je org v aplikaci (§6.7)
